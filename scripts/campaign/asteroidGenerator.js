const asteroidGen = extend(PlanetGenerator, {
    ...
    generate(tiles, sec){
        ...
        const Room = {
            ...
            connect(to){
                ...
                asteroidGen.brush(asteroidGen.pathfind(this.x, this.y, to.x, to.y, tile => (tile.solid() ? 5 : 0) + asteroidGen.noiseOct(tile.x, tile.y, 1, 1, 1 / nscl) * 60, Astar.manhattan), stroke);
            }
        };
        ...
        for(var i = 0; i < 360; i += angleStep){
            ...
            if(waterTiles <= 4 || (i + angleStep >= 360)){
                ...
                for(var j = 0; j < enemySpawns; j++){
                    ...
                    var espawn = setRoom(Math.floor(Tmp.v1.x), Math.floor(Tmp.v1.y), rand.random(2, 8));
                    roomseq.add(espawn);
                    enemies.add(espawn);
                };
        ...
        var ores = Seq.with(Vars.content.getByName(ContentType.block, "factory-ore-darkshade"));
        ...
        this.pass((x, y) => {
			/*if (block != Blocks.air && block != Blocks.space) {
                if (nearAir(x, y)) {
                    if (block == Blocks.saltWall && noise(x + 78, y, 4, 0.7f, 33f, 1f) > 0.52f) {
                        block = Blocks.graphiticWall;
                    } else if (block != Blocks.saltWall && noise(x + 782, y, 4, 0.8f, 38f, 1f) > 0.665f) {
        ...
        this.pass((x, y) => {
            ...
            if(this.floor != null && this.floor != Blocks.basalt && this.floor != Blocks.basalt && this.floor.asFloor().hasSurface()){
                ...
                if(noise > 0.93){
                    this.floor = noise > 0.95 ? Blocks.water : (this.floor == Blocks.darksand ? Blocks.magmarock : Blocks.hotrock);
                    this.ore = Blocks.air;
                }else if(noise > 0.67){
        ...
        state.rules.waveSpacing = Mathf.lerp(60 * 65 * 2, 60 * 60 * 1, Math.floor(Math.max(difficulty - waveTimeDec, 0) / 0.6))*2;
        state.rules.waves = this.sector.info.waves = true;
        state.rules.enemyCoreBuildRadius = 880;
...
asteroidGen.basegen = new BaseGenerator();
asteroidGen.scl = 5;
